...
function startSubscriptions() {
  ...
  if (isBuero(state.user.role) || isMonteur(state.user.role)) {
    // Buero/Admin sehen alle Auftraege; Monteure nur die, bei denen sie
    // als Empfaenger eingetragen sind (wird serverseitig durch
    // firestore.rules erzwungen, hier nur passend dazu abgefragt).
    const options = isBuero(state.user.role) ? {} : { forUid: state.user.uid };
...
function renderShellForRole() {
  ...
  $$('[data-nav]').forEach((btn) => {
    ...
    const allowed =
      view === 'lager' ||
      (view === 'auftraege' && (isBuero(role) || isMonteur(role))) ||
...
function materialRow(m, canManage, canWithdraw) {
  ...
  return `
    <tr class="${low ? 'row-warn' : ''}">
      <td>
        ${m.manufacturer ? `<div class="muted">${escapeHtml(m.manufacturer)}</div>` : ''}
        ${escapeHtml(m.name)}
        ${m.extraInfo ? `<div class="muted">${escapeHtml(m.extraInfo)}</div>` : ''}
        ${m.sku ? `<div class="muted">${escapeHtml(m.sku)}</div>` : ''}
      </td>
...
function openMaterialDialog(material) {
  ...
  slot.innerHTML = dialogShell('material-dialog', isEdit ? 'Material bearbeiten' : 'Material anlegen', `
    <form id="material-form">
      <label>Hersteller<input name="manufacturer" value="${escapeHtml(material?.manufacturer || '')}"></label>
      <label>Bezeichnung *<input name="name" value="${escapeHtml(material?.name || '')}" required></label>
      <label>Zusatzinfo<input name="extraInfo" placeholder="z.B. Farbe, Bus, Linienmelder" value="${escapeHtml(material?.extraInfo || '')}"></label>
...
function renderAuftraege() {
  ...
  root.innerHTML = `
    <div class="toolbar">
      ...
      ` : `<p class="muted">Deine zugeordneten Auftraege - nur zum Nachschauen.</p>`}
    </div>
    <div class="order-list">
      ${state.orders.map((o) => orderCard(o, canManage)).join('') || '<p class="empty">Noch keine Auftraege.</p>'}
    </div>
  `;
...
function orderCard(o, canManage) {
  ...
  return `
    <div class="order-card ${complete ? 'order-complete' : ''}" data-id="${o.id}">
      <div class="order-card-head">
        ...
        ${canManage ? `<button class="btn btn-small btn-danger btn-delete-order" data-id="${o.id}">Loeschen</button>` : ''}
      </div>
...
function openOrderDetail(order) {
  ...
  slot.innerHTML = dialogShell('order-detail-dialog', `Auftrag ${escapeHtml(order.orderNumber)}`, `
    ...
    <div class="position-checklist">
      ${order.positions.map((p) => positionReceiptRow(p, canManage)).join('')}
    </div>
    ${canManage ? '' : '<p class="muted">Nur Buero/Admin koennen Positionen als eingegangen markieren.</p>'}
    <div class="dialog-actions">
      <button type="button" class="btn" data-close>Schliessen</button>
    </div>
  `);
  wireDialogClose(slot);
  wireOrderDetailButtons(slot, order);
}

function positionReceiptRow(p, canManage) {
  const label = `${escapeHtml(p.description)} - ${fmtNum(p.quantityOrdered)} ${escapeHtml(p.unit)}`;
  if (p.received) {
    const sourceText =
      p.receivedSource === 'lager'
        ? `aus Lager: ${escapeHtml(p.receivedFromMaterialName || '')}`
        : 'aus Bestellung';
    return `
      <div class="position-item position-item-done">
        <div class="position-info">
          ${label}
          <div class="muted">Eingegangen ${sourceText}${p.receivedByName ? ' - ' + escapeHtml(p.receivedByName) : ''}</div>
        </div>
        ${canManage ? `<button type="button" class="btn btn-small btn-undo-position" data-pos="${p.id}">Rueckgaengig</button>` : ''}
      </div>
    `;
  }
  return `
    <div class="position-item">
      <div class="position-info">${label}</div>
      ${canManage ? `
        <div class="position-actions">
          <button type="button" class="btn btn-small btn-receive-order" data-pos="${p.id}">Aus Bestellung</button>
          <button type="button" class="btn btn-small btn-receive-stock" data-pos="${p.id}">Aus Lager</button>
        </div>
      ` : ''}
    </div>
  `;
...
function wireOrderDetailButtons(slot, order) {
  $$('.btn-undo-position', slot).forEach((b) => {
    b.addEventListener('click', async () => {
      try {
        await backend.setPositionReceived(order, b.dataset.pos, false);
      } catch (err) {
  ...
  $$('.btn-receive-order', slot).forEach((b) => {
    b.addEventListener('click', async () => {
      try {
        await backend.setPositionReceived(order, b.dataset.pos, true);
      } catch (err) {
        showToast(err.message);
      }
    });
  ...
  $$('.btn-receive-stock', slot).forEach((b) => {
    b.addEventListener('click', () => handleReceiveFromStock(order, b.dataset.pos));
  });
}

async function handleReceiveFromStock(order, positionId) {
  const position = order.positions.find((p) => p.id === positionId);
  if (!position) return;

  const searchName = cleanMaterialSearchName(position.description);
  const matches = findMatchingMaterials(state.materials, searchName);

  if (matches.length === 0) {
    showToast(`Kein passendes Material im Lager gefunden fuer "${searchName}". Bitte im Lager anlegen oder "Aus Bestellung" waehlen.`);
    return;
  }
  if (matches.length === 1) {
    await confirmReceiveFromStock(order, positionId, matches[0]);
    return;
  }
  openMaterialPickerDialog(matches, (chosen) => confirmReceiveFromStock(order, positionId, chosen));
}

async function confirmReceiveFromStock(order, positionId, material) {
  try {
    await backend.receivePositionFromStock(order, positionId, material);
    showToast(`"${material.name}" im Lager um die Menge verringert.`);
  } catch (err) {
    showToast(err.message);
  }
}

// Wird gezeigt, wenn mehrere Lager-Artikel zum Positionstext passen -
// der Nutzer waehlt aus, welcher konkret gemeint ist.
function openMaterialPickerDialog(matches, onChoose) {
  const slot = $('#dialog-slot');
  slot.innerHTML = dialogShell('material-picker-dialog', 'Welches Material?', `
    <p class="muted">Es wurden mehrere passende Artikel im Lager gefunden - bitte auswaehlen:</p>
    <div class="order-list">
      ${matches.map((m) => `
        <div class="order-card btn-pick-material" data-id="${m.id}">
          <strong>${escapeHtml(m.name)}</strong>
          <div class="muted">${fmtNum(m.quantity)} ${escapeHtml(m.unit || '')}${m.location ? ' - ' + escapeHtml(m.location) : ''}</div>
        </div>
      `).join('')}
    </div>
    <div class="dialog-actions">
  ...
  `);
  wireDialogClose(slot);
  $$('.btn-pick-material', slot).forEach((el) => {
    el.addEventListener('click', () => {
      const chosen = matches.find((m) => m.id === el.dataset.id);
      closeDialog();
      onChoose(chosen);
    });
  });
