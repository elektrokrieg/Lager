...
// Position als "aus dem Lager entnommen" statt "aus der Bestellung
// erhalten" markieren: verringert das gewaehlte Lager-Material um die
// bestellte Menge (nutzt dieselbe Pruefung/Bewegungsprotokollierung wie
// eine normale Entnahme) und markiert die Position danach als erhalten.
// Hinweis: Das sind zwei einzelne Schreibvorgaenge (Material, dann
// Auftrag) - bei einem Verbindungsabbruch genau dazwischen koennte in
// seltenen Faellen nur der Lagerbestand sinken, ohne dass die Position
// als erhalten markiert wird. Fuer dieses Lagersystem (kein Hochfrequenz-
// Mehrbenutzerbetrieb) ist das Risiko vertretbar.
export async function receivePositionFromStock(order, positionId, material) {
  const position = order.positions.find((p) => p.id === positionId);
  if (!position) throw new Error('Position nicht gefunden.');

  await withdrawStock(material, position.quantityOrdered, `Auftrag ${order.orderNumber || ''}: ${position.description}`.trim());
  ...
  const positions = order.positions.map((p) =>
    p.id === positionId
      ? {
          ...
          receivedBy: currentUserProfile.uid,
          receivedByName: currentUserProfile.displayName || currentUserProfile.email,
          receivedSource: 'lager',
          receivedFromMaterialId: material.id,
          receivedFromMaterialName: material.name,
        }
      : p
  );
  ...
  const positions = order.positions.map((p) =>
    p.id === positionId
      ? {
          ...
          // Herkunft "aus Lager" (falls vorher so markiert) beim
          // Zuruecksetzen bzw. beim manuellen "aus Bestellung" wieder
          // entfernen, damit keine veraltete Zuordnung stehen bleibt.
          receivedSource: received ? p.receivedSource || 'bestellung' : null,
          receivedFromMaterialId: received ? p.receivedFromMaterialId || null : null,
          receivedFromMaterialName: received ? p.receivedFromMaterialName || null : null,
        }
      : p
  );
  await updateOrder(order.id, { positions });
