...
export function parseImportText(rawText) {
  ...
  const customer = extractFirstMatch(text, [
    /Kunde\s*[:#]?\s*(.+)/i,
    /Kundenname\s*[:#]?\s*(.+)/i,
    /Auftraggeber\s*[:#]?\s*(.+)/i,
  ]);
...
const POSITION_LINE_RE = /^(\d+(?:[.,]\d+)?)\s*(stk|stck|st\.?|m|meter|kg|l|liter|pack|pkg|rolle|rl|dose|karton|kt)\b\.?\s+(.+)$/i;
const POSITION_LINE_RE_TRAILING = /^(.+?)\s+(\d+(?:[.,]\d+)?)\s*(stk|stck|st\.?|m|meter|kg|l|liter|pack|pkg|rolle|rl|dose|karton|kt)\.?$/i;

function extractPositions(lines) {
  const positions = [];
  for (const line of lines) {
    if (/^(Auftrag|Bestell|Angebot|Kunde|Auftraggeber|Datum|Seite)/i.test(line)) continue;

    let m = line.match(POSITION_LINE_RE);
    if (m) {
      positions.push({
